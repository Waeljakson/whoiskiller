...
function get(i){
 ...
 return {
  ...
  name:item.name,
  type:item.type,
  image:`assets/evidence/${item.img}.png`,
  key,
  result:key
   ? `تحليل ${item.name} يكشف معلومة مهمة مرتبطة بالتوقيت أو الفرصة أو الدافع، ويزيد أهمية ملف ${suspects[correct].name}.`
   : `${item.name} حرز حقيقي من مسرح الجريمة، لكنه لا يكفي منفردًا لتوجيه الاتهام.`
 };
});
 const allDocs=[
