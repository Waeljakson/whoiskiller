...
function analyze(id){AudioFx.evidence();const e=c().evidence.find(x=>x.id===id);S.evidence.add(id);$$(".lab").forEach(x=>x.innerHTML=`<strong>${esc(e.name)}</strong><p>${esc(e.result)}</p>`);renderEvidence();updatePhase()}
