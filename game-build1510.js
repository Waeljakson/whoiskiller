...
function renderSceneHotspots(){
 ...
 const cc=c(),spots=(cc.hotspots&&cc.hotspots.length?cc.hotspots:[]);
 const usable=spots.slice(0,Math.min(spots.length,cc.evidence.length));
 layer.innerHTML=usable.map((p,i)=>{
  const ev=cc.evidence[i];
  return `<button type="button" class="scene-hotspot cinematic-hotspot ${S.evidence.has(ev.id)?"found":""}" data-scene-evidence="${ev.id}" style="--x:${Number(p.x)}%;--y:${Number(p.y)}%;" aria-label="فحص ${esc(ev.name)}"><span class="hotspot-number">${i+1}</span><span class="hit-label">${esc(ev.name)}</span></button>`;
 }).join("");
}
